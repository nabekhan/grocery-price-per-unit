(() => {
  ...
  var state = { dimension: "auto", direction: "asc", restored: true, scope: null, revision: 0, timer: null };
  var storage = () => globalThis.browser?.storage || globalThis.chrome?.storage;
  var query = () => new URL(location.href).searchParams.get("q")?.trim().normalize("NFKC").replace(/\s+/g, " ").toLowerCase() || null;
  var scope = () => query() ? `query:${query()}` : `page:${location.pathname}${location.search}`;
  ...
  function normalizeProduct(value, id) {
    ...
    return bounded(value.name, 1500) && (price == null || Number.isFinite(price)) ? {
      ...
      name: value.name,
      currentPrice: price,
      unitPrice: bounded(value.unitPrice, 160),
      unitOfSize: value.unitOfSize
    } : null;
  }
  window.addEventListener("message", (event) => {
    const message = event.data;
    if (event.source !== window || event.origin !== location.origin || message?.source !== SOURCE || message?.version !== VERSION || message.type !== "api-products" || message.mode !== "snapshot" || message.context?.query !== query() || !Number.isSafeInteger(message.revision) || message.revision < state.revision || typeof message.products !== "object") return;
  ...
  function scan() {
    ...
    const sortable = grid.models.filter((model) => sorted.dimension === "total" ? Number.isFinite(model.currentPrice) : model.dimension === sorted.dimension && Number.isFinite(model.normalizedUnitPrice)).length;
  ...
  function schedule() {
    ...
    state.timer = setTimeout(scan, 160);
  }
