(() => {
  ...
  (function initializeLoblawApiCapture(global) {
    ...
    function contextScope(context) {
      return context?.query ? `query:${context.query}|store:${context.storeId || ""}` : `page:${context?.pagePath || ""}`;
    }
    ...
    function contextFromRequest(urlValue, body) {
      let urlQuery = null;
      try {
        urlQuery = new URL(urlValue, global.location.href).searchParams.get("search-bar");
      } catch {
      }
      if (typeof body !== "string" || body.length > 2e5) return { query: urlQuery };
      try {
        const parsed = JSON.parse(body);
        const query = parsed?.listingInfo?.filters?.["search-bar"]?.[0] || parsed?.searchRelatedInfo?.term || urlQuery;
      ...
      xhr.send = function observedSend(body) {
        if (this.__rcssSearchUrl) {
          ...
          this.addEventListener("load", () => {
            try {
              ingest(JSON.parse(this.responseText), context);
            } catch {
            }
          }, { once: true });
        }
    ...
    function readNextData() {
      ...
      try {
        return ingest(JSON.parse(element.textContent));
      } catch {
        return false;
      }
    }
