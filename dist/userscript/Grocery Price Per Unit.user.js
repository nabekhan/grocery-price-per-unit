 ...
 *
 * Architecture
 * 1. A page-world response observer accepts only each retailer's own bounded
 *    product records. Product facts are never reconstructed from card text.
 * 2. Retailer adapters match accepted product IDs to rendered result cards.
 * 3. The parser normalizes mass, volume, and count independently; unlike
 *    dimensions are never ranked against one another.
 * 4. The sorter owns only its annotations, inline order values, and narrowly
 *    recognized sponsored/ad visibility. Restore releases that ownership.
 * 5. The fixed control reports pending/no-match states instead of presenting a
 *    stale sort as current. Cookie prompts and CAPTCHAs remain human-only.
 *
 * Source of truth
...
if (hostname === 'www.realcanadiansuperstore.ca' || hostname === 'www.nofrills.ca') {
  /* Entry module: src/retailers/loblaw/api-capture-main.js
   * Bundled without minification so this installed artifact remains auditable.
   */
(() => {
  ...
  (function initializeLoblawApiCapture(global) {
    ...
    function isActiveRequest(context) {
      ...
      if (isLaterPage) {
        if (state.latestRequestScope) return context.scope === state.latestRequestScope && context.requestSequence > state.latestRequestSequence;
        return !state.scope || context.scope === state.scope;
      }
      return context.scope === state.latestRequestScope && context.requestSequence >= state.latestRequestSequence;
    }
    ...
    function ingest(payload, requestContext = null, { authoritative = false } = {}) {
      ...
      useScope(context);
      if (!isLaterPage) {
        const laterProducts = Object.fromEntries(Object.entries(state.products).filter(([id]) => (state.productSequences[id] || 0) > context.requestSequence));
        const laterSequences = Object.fromEntries(Object.entries(state.productSequences).filter(([, sequence]) => sequence > context.requestSequence));
  ...
  /* Entry module: src/retailers/loblaw/content.js
   * Bundled without minification so this installed artifact remains auditable.
   */
(() => {
  ...
  function updateStatus(root, { dimension, sortable, incompatible, unknown, total, excluded = 0, range = null, restored = false, dataState = null }) {
    ...
    const transitional = dataState === "pending" ? ["Waiting for current-page product data", preservation, ...loadedProducts ? [loadedProducts] : [], ...excluded ? [promotions] : []] : dataState === "no-match" ? ["No matching product data in these loaded results", preservation, ...loadedProducts ? [loadedProducts] : [], ...excluded ? [promotions] : []] : null;
  ...
  var installClaimed = claimRuntimeInstall("loblaw-content");
  ...
  function scan() {
    ...
    if (!grid) {
      ...
      if (control2) updateStatus(control2, state.restored ? { total: void 0, excluded: 0, restored: true } : { total: void 0, excluded: 0, dataState: "pending" });
      return;
    }
...
if (hostname === 'www.walmart.ca') {
  /* Entry module: src/retailers/walmart/userscript-main.js
   * Bundled without minification so this installed artifact remains auditable.
   */
(() => {
  ...
  (function initializeApiCapture(global) {
    ...
    function initialNextDataContext(target, payload) {
      ...
      return {
        ...
        storeId: storeIdentityFromPage(target.location && target.location.href, expectedOrigin) || storeIdentityFromVariables(variables).value,
        variablesFingerprint: variablesFingerprint(variables),
        pageUrlAtRequest: pageLocation.pagePath,
  ...
  var installClaimed = claimRuntimeInstall("walmart-content");
  ...
  var observedApiScope = currentApiScope();
  ...
  var installClaimed2 = claimRuntimeInstall("walmart-sort");
  ...
  function scan() {
    ...
    if (!found) {
      ...
      if (existingControl && !state.restored) updateStatus(existingControl, { total: void 0, dataState: "pending" });
      return;
    }
    const [grid] = found;
...
if (hostname === 'www.saveonfoods.com') {
  /* Entry module: src/retailers/saveon/api-capture-main.js
   * Bundled without minification so this installed artifact remains auditable.
   */
(() => {
  ...
  (function initializeSaveOnCapture(global) {
    ...
    function isActive(context) {
      ...
      if (context.isLaterPage) {
        if (state.latestRequestScope) return context.scope === state.latestRequestScope && context.sequence > state.latestRequestSequence;
        return !state.scope || context.scope === state.scope;
      }
    ...
    function ingestProducts(values, context = pageContext(), { authoritative = false } = {}) {
      ...
      for (const value of values.slice(0, MAX_CAPTURED_PRODUCTS)) {
        ...
        if (sequence2 >= (state.productSequences[product.id] || 0)) {
      ...
      if (!context.isLaterPage) {
        const laterProducts = Object.fromEntries(Object.entries(state.products).filter(([id]) => (state.productSequences[id] || 0) > sequence));
    ...
    function inspectResponse(response, context) {
      ...
      if (response?.url && (!finalUrl || normalizedQuery(finalUrl.searchParams.get("q")) !== context.query || endpointStoreId(finalUrl) !== context.storeId || requestFingerprint(finalUrl) !== context.filterFingerprint)) return;
      const contentType = response?.headers?.get?.("content-type");
        ...
        if (context) {
          this.addEventListener("load", () => {
            try {
              ...
              if (this.responseURL && (!finalUrl || normalizedQuery(finalUrl.searchParams.get("q")) !== context.query || endpointStoreId(finalUrl) !== context.storeId || requestFingerprint(finalUrl) !== context.filterFingerprint)) return;
              const contentType = this.getResponseHeader?.("content-type");
              if (contentType && !/\bjson\b/i.test(contentType)) return;
  ...
  /* Entry module: src/retailers/saveon/content.js
   * Bundled without minification so this installed artifact remains auditable.
   */
(() => {
  // src/parsing/units.js
  ...
  var VERSION = 2;
  var isProductArray = Array.isArray;
  var installClaimed = claimRuntimeInstall("saveon-content");
  var MAX_API_REVISION = 1e6;
  var MAX_API_REVISION_ADVANCE = 1e4;
  ...
  function scan() {
    ...
    if (excluded === null) {
      ...
      updateStatus(control, state.restored ? { total: grid.models.length, excluded: 0, restored: true } : { total: grid.models.length, excluded: 0, dataState: "pending" });
      return;
    }
    for (const model of grid.models) {
      if (model.dataSource === "api") annotate(model);
