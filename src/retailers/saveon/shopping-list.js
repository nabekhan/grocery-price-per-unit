...
function storeIdForSearch(url = new URL(location.href)) {
  return url.pathname.match(/\/rsid\/([^/]+)(?:\/|$)/i)?.[1] || null;
}
...
export async function querySaveOnProducts(query, { onProgress } = {}) {
  ...
  // A stalled gateway must release the shared runner for an in-place retry.
...
export function createSaveOnShoppingAdapter() {
  return createRetailerCartAdapter({
    ...
    searchUnavailableReason: 'Choose a Save-On-Foods store, then retry.',
    cartUnavailableReason: 'Save-On-Foods cart API is unavailable.',
