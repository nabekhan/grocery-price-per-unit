(function injectPageWorldCapture(global) {
  ...
  script.addEventListener('error', () => { delete root.dataset.sppuApiCaptureInjected; script.remove(); }, { once: true });
  root.prepend(script);
})(globalThis);
...
(function initializeSaveOnCapture(global) {
  ...
  function findProductArrays(value, found = [], seen = new Set(), depth = 0) {
    ...
    xhr.send = function observedSend(body) {
      if (this.__sppuApiResponse) {
        this.addEventListener('load', () => {
          try { ingestProducts(findProductArrays(JSON.parse(this.responseText))); } catch { /* ignored */ }
        }, { once: true });
      }
